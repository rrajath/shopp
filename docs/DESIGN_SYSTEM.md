...
**Flat 10 colors, the same values in both themes** -- from `ShoppApp.dc.html`'s own `PALETTE` constant, which (unlike the old prototype and the interim 15-color palette) is used identically regardless of theme:

| Index | Hex | Hue |
|---|---|---|
| 0 | `#C67139` | burnt orange |
| 1 | `#7A8A5E` | olive |
| 2 | `#B2622D` | terracotta |
| 3 | `#8FA073` | sage |
| 4 | `#D67F48` | light orange |
| 5 | `#728157` | deep olive |
| 6 | `#F6A06B` | peach |
| 7 | `#AEBF92` | light sage |
| 8 | `#8C491A` | dark brown |
...
`LabelColorAllocator`: lowest unused index, else `count mod 10`. **No user-facing color picker** -- auto-allocated only, matching the prototype (the interim 15-color palette + Labels "Edit" swatch picker, added August 2026, has been removed as part of this revert). A few entries (`#D67F48`, `#F6A06B`, `#AEBF92`) are low-contrast as small dots against a white background; accepted as literal fidelity to the prototype since they're only ever used for 8-9px dots, never text.
...
- **Section header** (`SectionHeader`): small bold uppercase micro-label (12px, `.08em`, 62%-ish `muted` text) with a leading 8dp identity-colored dot -- **not** the large 27px serif heading this used to be. The dot carries the section's color (label color, or the fixed `inboxTint` for Inbox); the text itself is always `muted`, never label-tinted. This resolves the "Known deliberate deviation" the previous revision of this doc flagged against PRD §11's "smaller/heavier" micro-header -- the new prototype turned out to specify exactly that.
...
- **Chip** (`LabelChipRow`): pill (`chipCornerRadius = 100.dp`, `chipMinHeight = 28.dp`, 12.5px text). Selected = **uniformly accent-filled** (`chipSelectedText`, SemiBold) regardless of which label it represents, Inbox included. Unselected = **filled** with `chipUnselectedFill` (not outlined), text in `chipUnselectedText` (a dedicated neutral-800 tone, not `foreground`), no colored dot. Per-label identity color shows only in section headers, the Labels screen, and the merge picker's list -- not in the chip row itself.
...
- Label colors are auto-allocated only (10-color fixed palette, no user picker) -- matches the prototype; noted here only because an interim August 2026 change had added a 15-color picker, since removed in the color revert below.
